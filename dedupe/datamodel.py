...
class DataModel(object):
    ...
    def _field_comparators(self):
        # TODO: What is the purpose of start, stop? - VSM
        # It looks like they are byte offsets for the field object that is
        # useful when pickling, but I can't tell.  What is len(field) when field
        # is an object?
        start = 0
        stop = 0
        comparators = []
    ...
    def distances(self, record_pairs):
        """distances computes the distance between all record pairs

        :param record_pairs: sequence of record pairs
        """
        num_record_pairs = len(record_pairs)
        LOG.debug('Computing distances between {n} record pairs'.format(n=num_record_pairs))
        distances = numpy.empty((num_record_pairs, len(self)), 'f4')
        field_comparators = self._field_comparators

        for i, (record_1, record_2) in enumerate(record_pairs):
            # TODO: how / why do start and stop get used in the context of
            # setting up the distances vector?
    ...
    def check(self, record):
        """check confirms that the given record conforms to this data model.  If
        not, a ValueError is raised.

        :param record: record to check schema for
        """
...
def typifyFields(fields):
    ...
    for definition in fields:
        ...
        if field_type == 'FuzzyCategorical' and \
                'other fields' not in definition:
